...
class InstanceRuntime:
    ...
    temporal_expert_log_dir: Optional[Path]
    command: List[str]
    env: Dict[str, str]
    ...
    *,
    ...
    if enable_moe_profiling:
        if moe_log_dir is None:
            raise ValueError("moe_log_dir must be set if profiling is enabled")
        cmd.extend([
    ...
    if enable_temporal_expert_logging:
        if temporal_expert_log_dir is None:
            raise ValueError(
                "temporal_expert_log_dir must be set if temporal expert logging is enabled"
            )
        cmd.extend([
            "--enable-temporal-expert-logging",
            "--temporal-expert-log-dir",
            str(temporal_expert_log_dir),
        ])
    ...
    cfg = _deep_merge_dict(
        {
            ...
            "temporal_expert_logging": {
                "enabled": False,
                "subdir": "temporal_expert_logs",
            },
            "global_env": {},
            "common_vllm_args": [],
            "gpu_pool": [],
        ...
        for idx in range(count):
            ...
            enable_moe_profiling = bool(
                override.get(
                    ...
                    cfg.get("moe_profiling", {}).get("enabled", False),
                )
            )
            moe_log_dir: Optional[Path] = None
            if enable_moe_profiling:
                moe_subdir = str(cfg.get("moe_profiling", {}).get("subdir", "moe_profiles"))
                moe_log_dir = instance_log_dir / moe_subdir
                moe_log_dir.mkdir(parents=True, exist_ok=True)

            enable_temporal_expert_logging = bool(
                override.get(
                    "enable_temporal_expert_logging",
                    cfg.get("temporal_expert_logging", {}).get("enabled", False),
                )
            )
            temporal_expert_log_dir: Optional[Path] = None
            if enable_temporal_expert_logging:
                temporal_subdir = str(
                    override.get(
                        "temporal_expert_log_subdir",
                        cfg.get("temporal_expert_logging", {}).get(
                            "subdir", "temporal_expert_logs"
                        ),
                    )
                )
                temporal_expert_log_dir = instance_log_dir / temporal_subdir
                temporal_expert_log_dir.mkdir(parents=True, exist_ok=True)
            ...
            all_instances.append(
                {
                    ...
                    "moe_log_dir": str(moe_log_dir) if moe_log_dir is not None else None,
                    "temporal_expert_log_dir": (
                        str(temporal_expert_log_dir)
                        if temporal_expert_log_dir is not None
                        else None
                    ),
                    "command": cmd,
                    "env": env,
...
def launch_instance(
    ...
    *,
    ...
    return InstanceRuntime(
        ...
        log_file=log_file,
        moe_log_dir=Path(instance_cfg["moe_log_dir"])
        if instance_cfg.get("moe_log_dir")
        else None,
        temporal_expert_log_dir=Path(instance_cfg["temporal_expert_log_dir"])
        if instance_cfg.get("temporal_expert_log_dir")
        else None,
        ...
        summary.append(
            {
                ...
                "moe_log_dir": str(rt.moe_log_dir) if rt.moe_log_dir else None,
                "temporal_expert_log_dir": (
                    str(rt.temporal_expert_log_dir)
                    if rt.temporal_expert_log_dir
                    else None
                ),
                "command": rt.command,
            }
